...
(function ($) {
    ...
    $("body").off("click", ".bb-qtybtn").on("click", ".bb-qtybtn", function () {
        var $qtybutton = $(this);
        var QtyoldValue = $qtybutton.parent().find("input").val();
        var priceValue = $qtybutton.closest("tr").find('.price').data('price');
        var QtynewVal;

        if ($qtybutton.text() === "+") {
            QtynewVal = parseFloat(QtyoldValue) + 1;
        } else {
            if (QtyoldValue > 1) {
                QtynewVal = parseFloat(QtyoldValue) - 1;
            } else {
        ...
        var totalPrice = priceValue * QtynewVal;
        $qtybutton.parent().find("input").val(QtynewVal);
        $qtybutton.closest("tr").find('.total-price-item').val(totalPrice);
        $qtybutton.closest("tr").find('.totalprice').text(totalPrice + "đ");
    ...
    function updateTotalPrice() {
        let total = 0;
        let totalcheckout = 0;
        // let freight = document.getElementById('freight-input').value;
        let freight = 30000;
